  ...
  module ResourcesHelper
    ...
    def resource_attribute_field_options(attribute)
      ...
      case attribute[:type].to_s
        ...
        when 'delegated_alchemy_element'
          options.merge(as: 'alchemy_element')
        else
          options.merge(as: 'string')
      end
    end
    ...
    #
    # #TODO abbiamo fatto questa correzione in attesa di una nuova release con bug corretto.
    # pre provare che non funziona devi fare una ricerca dalla essence image di un'immagine con la ricerca
    def current_location_params
      {
        q: request.params[:q],
        page: request.params[:page],
        tagged_with: request.params[:tagged_with],
